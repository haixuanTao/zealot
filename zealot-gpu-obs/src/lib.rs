...
/// `#[derive(Shader)]` supplies `from_backend` for the stacking entry.
#[derive(Shader)]
struct ObsStackShader {
    kernel: shaders::GpuObsStack,
}

/// Device obs-history stacking (see `gpu_obs_stack`): fresh frame + previous
/// stacked arena slot → new stacked arena slot, with reset replication.
...
impl GpuObsStack {
    pub fn new(
        ...
        Ok(Self {
            ...
            fresh: Tensor::vector(
                backend,
                &vec![1u32; n], // first step: no prev — replicate
                BufferUsages::STORAGE | BufferUsages::COPY_DST,
            )?,
            params: Tensor::scalar(
                backend,
                glamx::UVec4::new(n as u32, frame, hist, 0),
    ...
    /// Upload this step's per-env fresh flags (1 = episode reset since the
    /// previous step → replicate the frame into all history slots).
    pub fn set_fresh(
        &mut self,
        backend: &GpuBackend,
        fresh: &[u32],
    ) -> Result<(), GpuBackendError> {
        debug_assert_eq!(fresh.len(), self.n);
        backend.write_buffer(self.fresh.buffer_mut(), 0, fresh)
    }

    /// Encode: `frame` `[F × n]` + `prev` stacked `[H·F × n]` → `out`.
    pub fn encode(
        &mut self,
        enc: &mut Enc,
        frame: &Tensor<f32>,
        ...
        let mut pass = enc.begin_pass("obs_stack", None);
        self.shader.kernel.call(
            &mut pass,
            [self.n as u32, self.stacked, 1],
            frame,
            prev,
            &self.fresh,
            out,
...
/// `#[derive(Shader)]` supplies `from_backend` for the Welford entry.
#[derive(Shader)]
struct WelfordShader {
    kernel: shaders::GpuWelford,
}

/// Device Welford accumulation of the obs-normalizer statistics (see
/// `gpu_welford`) — bit-identical arithmetic to the host `PendingNorm::push`
/// loop. Read the `[3 × dim]` moments at the iteration commit.
pub struct GpuWelford {
    shader: WelfordShader,
    state: Tensor<f32>,
    params: Tensor<glamx::UVec4>,
    dim: usize,
}
...
impl GpuWelford {
    pub fn new(backend: &GpuBackend, dim: usize, n: usize) -> Result<Self, GpuBackendError> {
        Ok(Self {
            shader: WelfordShader::from_backend(backend)?,
            state: Tensor::vector(
                backend,
                &vec![0.0f32; 3 * dim],
            ...
            params: Tensor::scalar(
                backend,
                glamx::UVec4::new(dim as u32, n as u32, 0, 0),
                BufferUsages::UNIFORM | BufferUsages::STORAGE,
            )?,
            dim,
        })
    ...
    /// Encode one step's accumulation over `data` `[dim × n]`.
    pub fn encode(&mut self, enc: &mut Enc, data: &Tensor<f32>) -> Result<(), GpuBackendError> {
        let mut pass = enc.begin_pass("welford", None);
        self.shader
            .kernel
            .call(&mut pass, [self.dim as u32, 1, 1], data, &mut self.state, &self.params)
    }

    /// Read the accumulated `(count, mean, m2)` and zero the device state for
    /// the next iteration.
    pub async fn take_moments(
        &mut self,
        backend: &GpuBackend,
    ) -> Result<(f32, Vec<f32>, Vec<f32>), GpuBackendError> {
        let all: Vec<f32> = backend.slow_read_vec(self.state.buffer()).await?;
        backend.write_buffer(self.state.buffer_mut(), 0, &vec![0.0f32; 3 * self.dim])?;
        let count = all[0];
