...
cd "$ROOT"
cargo build --release --bin iter_e2e_bench --features "gpu biped_gpu" 2>&1 | tail -1
...
cargo build --release --bin iter_e2e_bench --features "gpu biped_gpu cuda_backend cutile" 2>&1 | tail -1
...
run_bench webgpu-12dof "$ROOT/target/release/iter_e2e_bench" KHAL_BACKEND=webgpu

echo "=== STAGE 6: CUDA+cuTile bold column (AGILE-parity realism+terrain) ===" | tee -a "$OUT/raw.log"
run_bench cuda-bold "$ROOT/target-cuda/release/iter_e2e_bench" \
...
run_bench cuda-production "$ROOT/target-cuda/release/iter_e2e_bench" \
