...
fn main() {
    ...
    let gpu_iter_ms = pollster::block_on(async {
        ...
        gpu.set_cutile(ct);
        gpu.init_raw_batch(env.backend(), t_steps)
            .expect("raw batch buffers");
        {
            let bk = env.backend();
            ...
            let idp = |d: usize| {
                Tensor::vector(
                    bk,
                    &(0..d as u32).collect::<Vec<u32>>(),
                    BufferUsages::STORAGE,
                )
                .unwrap()
            };
            let ones =
                |d: usize| Tensor::vector(bk, &vec![1.0f32; d], BufferUsages::STORAGE).unwrap();
        ...
        let _ = gpu
            .forward(env.backend(), &ac2, &gc, &gcc, 0)
            .await
            .unwrap();
        let mut samp: Vec<Vec<Sample>> = (0..n).map(|_| Vec::with_capacity(t_steps)).collect();
        let (mut rs, mut vs, mut ds): (Vec<Vec<f32>>, Vec<Vec<f32>>, Vec<Vec<bool>>) = (
        ...
        for step_i in 0..t_steps {
            ...
            let (means, values) = gpu
                .forward(env.backend(), &ac2, &gc, &gcc, step_i)
                .await
                .unwrap();
        ...
        if std::env::var("BIPED_IDLE_DBG").is_ok() {
            ...
            eprintln!(
                ...
                ms(et.stage_motors_ns as u128),
                ms(et.flush_static_ns as u128),
                ms(et.pipeline_step_ns as u128),
                ms(et.gpu_wait_ns as u128),
                ms(et.readback_ns as u128),
                ms(et.par_compute_ns as u128),
