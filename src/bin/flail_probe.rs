...
fn tau_table() -> ([f32; NUM_JOINTS], [f32; NUM_JOINTS], [f32; NUM_JOINTS], [f32; NUM_JOINTS], [f32; NUM_JOINTS]) {
...
fn main() {
    ...
    let steps: usize = std::env::args().nth(2).and_then(|s| s.parse().ok()).unwrap_or(150);
    let scale: f32 = std::env::args().nth(3).and_then(|s| s.parse().ok()).unwrap_or(1.0);
    ...
    pollster::block_on(async {
        ...
        for t in 0..steps {
            ...
            for e in 0..n {
                ...
                for k in 0..NUM_JOINTS {
                    ...
                    pw.push((tau * v).abs());
                    let b = if va < 10.0 { 0 } else if va < 15.0 { 1 } else if va < 20.0 { 2 } else { 3 };
        ...
        println!(
            ...
            pct(0.50), pct(0.90), pct(0.99), pct(0.999), qd_abs[qd_abs.len() - 1]
        );
        let over = |thr: f32| qd_abs.iter().filter(|v| **v > thr).count() as f64 / qd_abs.len() as f64 * 100.0;
        println!(
            "tail fractions: >10={:.3}% >20={:.3}% >30={:.4}% >50={:.4}%",
            over(10.0), over(20.0), over(30.0), over(50.0)
        );
        println!("per-joint max: {:?}", per_joint_max.map(|v| (v * 10.0).round() / 10.0));
        ...
        println!(
            ...
            ppct(0.90), ppct(0.99), ppct(0.999), pw[pw.len() - 1],
            pw.iter().filter(|v| **v > 1500.0).count() as f64 / pw.len() as f64 * 100.0,
            pw.iter().filter(|v| **v > 3000.0).count() as f64 / pw.len() as f64 * 100.0
        );
        for i in 0..6 {
            if ssr_n[i] > 0 {
                println!("  steps-since-reset {}{}: mean|qd| {:.2}  max {:.1}  (n={})",
                    i, if i==5 {"+"} else {""}, ssr_sum[i]/ssr_n[i] as f64, ssr_max[i], ssr_n[i]);
            }
        }
        if g_up_n > 0 {
            println!("POST-RESET UPRIGHT: mean grav-up {:.3} ({} samples), NOT-upright(>-0.5): {:.1}%",
                g_up_sum / g_up_n as f64, g_up_n, g_up_bad as f64 / g_up_n as f64 * 100.0);
        }
        if !ep_lens.is_empty() {
            ep_lens.sort();
            let e = &ep_lens;
            ...
            println!(
                ...
                e.len(), q(0.10), q(0.50), q(0.90), frac_short
            );
        }
        for (i, name) in ["<10", "10-15", "15-20", ">20"].iter().enumerate() {
            if band_n[i] > 0 {
                println!("  mean|tau| @ |qd| {name} rad/s: {:.1} N.m (n={})", band_sum[i] / band_n[i] as f64, band_n[i]);
            }
        }
    });
}
