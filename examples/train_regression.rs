...
async fn main() -> anyhow::Result<()> {
    ...
    // Fixed sample + target.
    let gx = mk(
        &DMatrix::from_fn(IN, 1, |r, _| (r % 4) as f32 * 0.25 - 0.5),
        st,
    );
    ...
    for t in 1..=STEPS {
        ...
        // Adam updates (grads read before params change; param tensors are distinct from grads)
        {
            ...
            adam.step(
                &backend, &mut sh, &mut p, &gp, &mut gw2, &gdw2, &mut mw2, &mut vw2,
            )?;
        }
        {
            let mut p = enc.begin_pass("adam_b2", None);
            adam.step(
                &backend, &mut sh, &mut p, &gp, &mut gb2, &z2, &mut mb2, &mut vb2,
            )?;
        }
        {
            let mut p = enc.begin_pass("adam_w1", None);
            adam.step(
                &backend, &mut sh, &mut p, &gp, &mut gw1, &gdw1, &mut mw1, &mut vw1,
            )?;
        }
        {
            let mut p = enc.begin_pass("adam_b1", None);
            adam.step(
                &backend, &mut sh, &mut p, &gp, &mut gb1, &gda1, &mut mb1, &mut vb1,
            )?;
        }
        backend.submit(enc)?;
        backend.synchronize()?;
    ...
    anyhow::ensure!(
        first_loss > 1e-3,
        "initial loss unexpectedly tiny; test not meaningful"
    );
