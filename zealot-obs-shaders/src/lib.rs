...
/// Obs-history stacking, on device: append the fresh frame `[F × n]` to the
/// previous STACKED slot `[H·F × n]` (oldest→newest, matching
/// `ObsHistory::write_stacked`) into the new stacked slot — drop the oldest
/// frame, shift, append. `fresh[e] != 0` (episode reset last step)
/// replicates the fresh frame into all H slots, matching
/// `ObsHistory::reset` + `reset_stacked`.
///
/// Dispatch `[n, H·F, 1]` threads.
#[spirv_bindgen]
#[spirv(compute(threads(64)))]
pub fn gpu_obs_stack(
    #[spirv(global_invocation_id)] invocation_id: UVec3,
    #[spirv(storage_buffer, descriptor_set = 0, binding = 0)] frame: &[f32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 1)] prev: &[f32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 2)] fresh: &[u32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 3)] out: &mut [f32],
    // x = n_envs, y = frame dim F, z = history H.
    ...
    let block = d / f;
    let k = d % f;
    let v = if fresh.read(e as usize) != 0 || block == h - 1 {
        frame.read((k * n + e) as usize)
    } else {
        prev.read(((block + 1) * f + k) as usize * n as usize + e as usize)
    };
    out.write((d * n + e) as usize, v);
}

/// Welford accumulation of the running obs-normalizer statistics, on device:
/// one thread per DIM, iterating the envs SEQUENTIALLY in index order — the
/// exact per-dim arithmetic sequence of the host `PendingNorm::push` loop
/// (`for e { for d { .. } }` visits envs in the same order per dim), so the
/// merged statistics are bit-identical to the host path.
///
/// `state` is `[3 × dim]` row-major: count / mean / m2 per dim (count is
/// replicated across dims — same value, keeps the kernel free of cross-thread
/// coordination). Dispatch `[dim, 1, 1]` threads.
#[spirv_bindgen]
#[spirv(compute(threads(64)))]
pub fn gpu_welford(
    #[spirv(global_invocation_id)] invocation_id: UVec3,
    #[spirv(storage_buffer, descriptor_set = 0, binding = 0)] data: &[f32],
    #[spirv(storage_buffer, descriptor_set = 0, binding = 1)] state: &mut [f32],
    // x = dim, y = n_envs.
    ...
    let mut count = state.read(d as usize);
    let mut mean = state.read((dim + d) as usize);
    let mut m2 = state.read((2 * dim + d) as usize);
    let mut e = 0u32;
    while e < n {
        let x = data.read((d * n + e) as usize);
        count += 1.0;
        let delta = x - mean;
        mean += delta / count;
        m2 += delta * (x - mean);
